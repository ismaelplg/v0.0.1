  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-background p-4 md:p-8">
      <div className="relative w-full max-w-4xl">
        {/* Slide Container */}
        <div className="aspect-[16/9] w-full overflow-hidden rounded-2xl border border-border bg-card shadow-lg">
          <div className="flex h-full flex-col items-center justify-center p-8 md:p-16">
            {slide.type === "title" && (
              <div className="text-center">
                ...
                <p className="mt-4 font-mono text-xl text-primary md:text-2xl">
                  {slide.content.version}
                </p>
                <div className="mt-8 flex flex-col gap-2 text-muted-foreground">
                  <p className="text-sm md:text-base">{slide.content.dates}</p>
                        ...
                        (
                          ...
                          return (
                            <div key={index} className="flex items-center">
                              <div className="w-40 shrink-0 truncate pr-3 text-xs text-foreground md:text-sm">
                                {task.name}
                              </div>
                              <div className="relative flex flex-1">
                                {Array.from({ length: columns }).map((_, colIndex) => {
                                  ...
                                  return (
                                    <div
                                      key={colIndex}
                                      className="relative flex h-7 flex-1 items-center border-l border-border/30 last:border-r"
                                    >
                                      {isInRange && (
                                        <div
                              ...
                              <div className="w-12 shrink-0 pl-2 text-right font-mono text-xs text-muted-foreground">
                                {task.hours}
                              </div>
                            </div>
                          )
                  ...
                  {slide.content.tasks.map((task, index) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm text-foreground md:text-base">{task.name}</span>
                      </div>
                      <div className="flex items-center gap-4">
                        <span className="font-mono text-xs text-muted-foreground md:text-sm">{task.hours}</span>
                        <span
            ...
            {slide.type === "milestones" && (
              <div className="w-full max-w-lg">
                ...
                <div className="relative space-y-8">
                  {/* Vertical line */}
                  <div className="absolute bottom-4 left-3 top-4 w-px bg-primary/30" />
            ...
            {slide.type === "swapping" && (
              <div className="w-full max-w-lg text-center">
                ...
                <p className="mb-4 font-mono text-lg text-primary md:text-xl">
                ...
                <div className="rounded-lg border border-primary/30 bg-primary/5 px-4 py-2">
                  <p className="text-xs text-muted-foreground">Deploy estimado</p>
            ...
            {slide.type === "roadmap" && (
              <div className="w-full max-w-lg text-center">
                ...
                <p className="mb-8 font-mono text-lg text-primary md:text-xl">
                  {slide.content.subtitle}
                </p>
                <div className="space-y-4">
                  {slide.content.milestones.map(
                    (milestone: { label: string; date: string; status: string }, index: number) => (
                      <div
                        key={index}
                        className="rounded-xl border-2 border-primary bg-primary/5 px-6 py-4"
                      >
                        ...
                        <p className="mt-2 font-mono text-base text-primary md:text-lg">
                          {milestone.date}
                        </p>
                      </div>
                    )
            ...
            {slide.type === "nextsteps" && (
              <div className="w-full max-w-2xl">
                ...
                <p className="mb-2 text-center font-mono text-lg text-primary md:text-xl">
                  {slide.content.subtitle}
                </p>
        ...
        <Button
          ...
          size="icon"
          onClick={prevSlide}
          className="absolute -left-4 top-1/2 -translate-y-1/2 rounded-full bg-card shadow-md hover:bg-muted md:-left-6"
        >
        ...
        <Button
          ...
          size="icon"
          onClick={nextSlide}
          className="absolute -right-4 top-1/2 -translate-y-1/2 rounded-full bg-card shadow-md hover:bg-muted md:-right-6"
        >
          ...
          {slides.map((_, index) => (
            <button
              ...
              className={`h-2 rounded-full transition-all ${
                index === currentSlide ? "w-8 bg-primary" : "w-2 bg-primary/20 hover:bg-primary/40"
              }`}
            >
